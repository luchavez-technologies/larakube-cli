...
## 🆕 Secrets Out of CI — Manual Test Guide (2026-08-04 session)

Covers `plans/active/secrets-out-of-ci.md`'s implementation: `dotenv:push`,
`dotenv:pull` (both new), `dotenv --strict` (new flag on the existing
`dotenv` command), `secrets:grant`/`secrets:revoke` (new, per-app/per-env
OpenBao access), and the CI workflow rewrite (GHA + GitLab) that stops
uploading ANY `.env` content as a CI secret — public/build vars are now baked
as literal `echo` lines into the generated workflow file itself. The
automated suite (1333 passing, `Process`/`Http` faked throughout) has never
actually round-tripped against a real cluster, real OpenBao, or a real CI
run — this is that walkthrough. Ordered by blast radius: free/local first,
the real GitHub Actions run (💰 in the sense of burning real CI minutes and
touching a real repo) last.
...
### Secrets Out of CI manual test phases (validate before v1.2.0 tags — see full guide above)
- [ ] **#29 — Phase A: `dotenv:push`/`dotenv:pull`, no OpenBao.** Per-key push/pull round-trip against a plain `laravel-secrets` Secret; locked-file skip.
- [ ] **#30 — Phase B: `dotenv --strict`.** Exits 1 on real drift, exits 0 (not 1) on a Plex/OpenBao-rotated key that legitimately differs — this is the one that matters, don't skip it.
- [ ] **#31 — Phase C: zero-blob CI generation.** Inspect a freshly-generated GHA workflow AND GitLab pipeline — confirm no `ENV_FILE_BASE64`/`BUILD_ENV_BASE64` anywhere, confirm literal `echo` lines carry real `VITE_*`/Reverb values.
- [ ] **#32 — Phase D: `dotenv:push`/`dotenv:pull` WITH OpenBao.** App-scoped KV path, the fixed `ExternalSecret` actually syncs (`status.conditions: Ready`), two apps sharing a cluster don't see each other's keys.
- [ ] **#33 — Phase E: `secrets:grant`/`secrets:revoke`.** A granted developer/viewer can log into OpenBao's UI and is confined to exactly `secret/data/{env}/{app}/*`; revoke actually cuts access.
- [ ] **#34 — Phase F: `sso:revoke` incident sweep covers app-scoped grants.** One `sso:revoke --email=` wipes both a global tier AND a `secrets:grant` grant in one pass.
- [ ] **#35 — Phase G: 💰 real GitHub Actions run.** Deploy job hard-fails without a prior `dotenv:push`, succeeds after; no secret value ever appears in CI logs; deployed app actually works.
...
- [ ] **#36 — Secrets Out of CI docs** — `dotenv:push`/`dotenv:pull`/`dotenv --strict`/`secrets:grant`/`secrets:revoke` are all new user-facing surface with no docs page yet; fold into #27's sweep. Also update whatever page currently documents the GHA/GitLab secrets setup (it still describes uploading an env-file secret) to reflect the zero-blob design.
...
# LaraKube Testing Suite

This document outlines the standard "Lifecycle Battle Test" required for major changes to LaraKube. These tests verify the entire project journey: from scaffolding to deployment, data persistence, and clean teardown.

## 🧪 Lifecycle Test Scenarios

Run these tests individually to verify each frontend stack.

### 1. React Stack
```bash
./laravel-k8s-cli/larakube new react-test --fast --react --no-boost --phpunit
cd react-test
../laravel-k8s-cli/larakube up --migrate --no-test --no-dashboard
../laravel-k8s-cli/larakube down --force
../laravel-k8s-cli/larakube up --migrate --no-test --no-dashboard
../laravel-k8s-cli/larakube reset --force
cd .. && rm -rf react-test
```

### 2. Vue Stack
```bash
./laravel-k8s-cli/larakube new vue-test --fast --vue --no-boost --phpunit
cd vue-test
../laravel-k8s-cli/larakube up --migrate --no-test --no-dashboard
../laravel-k8s-cli/larakube down --force
../laravel-k8s-cli/larakube up --migrate --no-test --no-dashboard
...
### 3. Svelte Stack
```bash
./laravel-k8s-cli/larakube new svelte-test --fast --svelte --no-boost --phpunit
cd svelte-test
../laravel-k8s-cli/larakube up --migrate --no-test --no-dashboard
../laravel-k8s-cli/larakube down --force
../laravel-k8s-cli/larakube up --migrate --no-test --no-dashboard
../laravel-k8s-cli/larakube reset --force
cd .. && rm -rf svelte-test
```

### 4. Livewire Stack
```bash
./laravel-k8s-cli/larakube new livewire-test --fast --livewire --no-boost --phpunit
cd livewire-test
../laravel-k8s-cli/larakube up --migrate --no-test --no-dashboard
../laravel-k8s-cli/larakube down --force
../laravel-k8s-cli/larakube up --migrate --no-test --no-dashboard
../laravel-k8s-cli/larakube reset --force
cd .. && rm -rf livewire-test
```

---

## 🚦 What these tests verify:
1.  **Installer Passthrough**: Ensures flags like `--react` reach the official Laravel installer.
2.  **Manifest Integrity**: Verifies Kustomize registration without "Already Registered ID" errors.
3.  **Logical Wait Order**: Confirms the CLI waits for databases before application pods.
4.  **Data Persistence**: The `down` -> `up` sequence verifies that local volume data survives a simple teardown.
5.  **Nuclear Reset**: Confirms `larakube reset` successfully wipes all state for the next developer.
