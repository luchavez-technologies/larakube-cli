...
## 🆕 Secrets Out of CI — see standalone test plan

Moved to its own file: `plans/active/secrets-out-of-ci-testing.md` — this
tracker was already too large/multi-era to be a good home for a single
feature's test plan. Tracker items #29–#30 below just point at it.
...
- [ ] **#30 — Secrets Out of CI docs** — own plan, `plans/active/secrets-out-of-ci-docs.md`. `github-actions.md` still actively describes the old (wrong) `{ENV}_ENV_FILE_BASE64` design; `dotenv:push`/`dotenv:pull`/`dotenv --strict`/`secrets:grant`/`secrets:revoke` have no docs page at all.
