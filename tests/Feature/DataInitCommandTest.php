...
test('data:init deploys Directus with Postgres, Redis, and SeaweedFS S3', function (): void {
    Process::fake([
        ...
        '*apply*' => Process::result(output: 'deployment.apps/directus created'),
...
test('data manifest wires the Commons Redis via the generic REDIS var, not CACHE_REDIS', function (): void {
    ...
    $manifest = view('k8s.data.shared', [
        'engine' => 'directus',
        'deployName' => 'directus-data-example-test',
        'secretName' => 'directus-secrets-data-example-test',
        'smtpSecretName' => 'directus-smtp-data-example-test',
        'oidcSecretName' => 'directus-oidc-data-example-test',
        'dbName' => 'directus_data_example_test',
        'bucket' => 'directus-storage-data-example-test',
...
test('data:init returns a failing exit code and does not claim success when kubectl apply is rejected', function (): void {
    ...
    Process::fake([
        ...
        '*exec*deploy/postgres*' => Process::result(output: 'CREATE DATABASE'),
        '*exec*deploy/seaweedfs*' => Process::result(output: 'bucket created'),
        '*apply*' => Process::result(output: 'The Deployment is invalid', exitCode: 1),
        '*rollout status*' => Process::result(output: 'deployment successfully rolled out'),
    ]);
...
function fakeDataInitProcess(bool $ssoWired, ?string &$appliedManifest): void
{
    Process::fake(function ($process) use ($ssoWired, &$appliedManifest) {
        ...
        if (str_contains($cmd, 'apply -f')) {
            ...
            $path = trim($m[1] ?? '', '\'"');
            if ($path !== '' && file_exists($path) && (str_contains($path, 'larakube-directus') || str_contains($path, 'larakube-pocketbase') || str_contains($path, 'larakube-data'))) {
