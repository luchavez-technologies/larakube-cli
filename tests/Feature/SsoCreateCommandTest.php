...
test('sso:create is registered', function () {
    $this->artisan('list --no-interaction')
        ->assertExitCode(0)
        ->expectsOutputToContain('sso:create');
});

test('sso:create provisions a new human user in Zitadel', function () {
    Process::fake([
...
test('sso:create reports if user already exists', function () {
    Process::fake([
        '*get deployment sso-zitadel*' => Process::result(output: 'sso-zitadel   1/1   1   1   10d'),
        '*get secret sso-secrets*' => Process::result(output: base64_encode('zitadel-pat')),
        '*get ingress sso-zitadel*' => Process::result(output: 'sso.example.com'),
