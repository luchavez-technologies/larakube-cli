...
test('a missing DO token fails clearly under --no-interaction instead of prompting', function (): void {
...
test('--stack-name skips the prompt and is slugified', function (): void {
    expect(cloudCreateFlagRunner(['--stack-name' => 'My App Stack!'])->stackName('ignored', 'vps'))
        ->toBe('my-app-stack');
});

test('--admin-cidr accepts a bare IPv4 (auto /32), a CIDR, and rejects garbage', function (): void {
...
test('no --admin-cidr headlessly means open (matches the confirm default of false)', function (): void {
