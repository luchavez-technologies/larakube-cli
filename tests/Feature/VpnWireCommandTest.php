...
test('vpn:wire rejects the public infrastructure tools per the VPN standardization plan', function (string $slug) {
...
test('vpn:wire creates the Middleware and re-applies the ingress with --vpn-only', function () {
    ...
    expect($exit)->toBe(0)
        ...
        ->and($command->calledWith['arguments']['--vpn-only'])->toBe(true)
        ->and($command->calledWith['arguments']['--no-interaction'])->toBe(true);
});
