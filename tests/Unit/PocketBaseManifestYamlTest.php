...
test('pocketbase manifest renders as valid multi-document YAML', function (): void {
...
test('pocketbase manifest includes a hooks ConfigMap that bridges mail/SSO env vars', function (): void {
    $documents = pocketbaseDocuments(pocketbaseManifest());

    $configMap = collect($documents)->firstWhere('kind', 'ConfigMap');
    expect($configMap)->not->toBeNull()
        ->and($configMap['metadata']['name'])->toBe('data-pocketbase-hooks');
...
test('pocketbase deployment mounts the hooks ConfigMap at /pb_hooks', function (): void {
    $documents = pocketbaseDocuments(pocketbaseManifest());
    ...
    expect($hookMount)->not->toBeNull()
        ->and($hookMount['name'])->toBe('pb-hooks');

    $volume = collect($deployment['spec']['template']['spec']['volumes'])->firstWhere('name', 'pb-hooks');
    expect($volume)->not->toBeNull()
        ->and($volume['configMap']['name'])->toBe('data-pocketbase-hooks');
});

test('pocketbase manifest renders valid YAML for local and vpn-only variants', function (): void {
    expect(pocketbaseDocuments(pocketbaseManifest(['isLocal' => true])))->not->toBeEmpty()
        ->and(pocketbaseDocuments(pocketbaseManifest(['isLocal' => false, 'vpnOnly' => true])))->not->toBeEmpty()
        ->and(pocketbaseDocuments(pocketbaseManifest(['isLocal' => true, 'vpnOnly' => true])))->not->toBeEmpty();
});

test('pocketbase deployment relies on the image\'s own entrypoint, not a broken custom command', function (): void {
