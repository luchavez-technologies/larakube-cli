 ...
 */
test('resolves an exact PRIMARY component match', function () {
    $match = ClusterTool::forDeployment('forgejo');
    expect($match['tool'])->toBe(ClusterTool::GIT)
        ->and($match['component']->key)->toBe('server');
});

test('a WORKER whose name prefixes another tool\'s deployment is never mistaken for an instance-suffixed copy of it', function () {
...
test('resolves an instance-suffixed Deployment to its base component', function () {
...
test('checks every engine variant, not just the default', function () {
...
test('returns null for an unmanaged Deployment — the exclusion mechanism itself', function () {
    expect(ClusterTool::forDeployment('prometheus-server'))->toBeNull();
});

test('a compound tool\'s bundled-storage component resolves by its own exact name', function () {
