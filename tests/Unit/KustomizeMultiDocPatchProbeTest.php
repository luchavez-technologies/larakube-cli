...
test('kustomizeHandlesMultiDocPatches is true when the build succeeds with non-empty output', function () {
    Process::fake(['kubectl kustomize *' => "apiVersion: apps/v1\nkind: Deployment\n"]);

    expect(kustomizeProbe()->handles('kubectl kustomize'))->toBeTrue();
});

test('kustomizeHandlesMultiDocPatches is false when the build fails', function () {
    Process::fake(['kubectl kustomize *' => Process::result(output: '', exitCode: 1)]);

    expect(kustomizeProbe()->handles('kubectl kustomize'))->toBeFalse();
});

test('kustomizeHandlesMultiDocPatches is false when the build succeeds but produces no output', function () {
