...
test('local env with no --context resolves to null (current kubectl context)', function () {
    $config = ConfigData::from(['blueprints' => []]);

    expect(contextOverrideHelper(null)->resolve($config, 'local'))->toBeNull();
});

test('local env with --context uses the override instead of null', function () {
    $config = ConfigData::from(['blueprints' => []]);

    expect(contextOverrideHelper('orbstack')->resolve($config, 'local'))->toBe('orbstack');
});

test('cloud env with no --context resolves to the saved cloud context', function () {
    $config = ConfigData::from(['blueprints' => []]);
...
test('cloud env with --context uses the override instead of the saved cloud context', function () {
    $config = ConfigData::from(['blueprints' => []]);
    $config->setCloud('production', new CloudData(ip: '159.89.205.239'));
