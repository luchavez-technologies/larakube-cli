 ...
 *
 * The volume inventory is discovered live from every `larakube-*` namespace's
 * Deployments, resolved back to a ClusterTool + component via
 * ClusterTool::forDeployment() (see backupVolumeTargets()) — the same
 * "read live rather than hardcoded" discipline backupDatabases() already
 * uses, so a newly installed tool is covered without a code change instead
 * of silently not being backed up. What used to be a hardcoded allow-list
 * comment (Prometheus/webmail/Synapse-bulk-media excluded) is now expressed
 * declaratively via each component's ClusterToolComponentData::$backupVolume
 * flag — see ClusterTool::components()'s own docblock for exactly which
 * components opt in and why.
 *
...
trait InteractsWithBackup
{
    ...
    /**
     * Legacy volume `name`s from the era backupVolumeTargets() was a
     * hardcoded array, keyed "{tool}:{component key}". A backup taken before
     * dynamic discovery wrote its manifest with these exact names — restoring
     * it does a lookup by name (see BackupRestoreCommand::restoreVolume()),
     * so changing them would silently strand every backup taken before this
     * became dynamic. New components default to "{tool}-{key}" instead;
     * only add an entry here to preserve a name that predates this map.
     *
     * @var array<string, string>
     */
    private const LEGACY_VOLUME_NAMES = [
        'secrets:app' => 'openbao',
        'git:server' => 'forgejo',
        'drive:app' => 'drive-ocis',
        'passwords:app' => 'vaultwarden',
        'mail:app' => 'stalwart',
        'chat:synapse' => 'synapse-identity',
    ];
     ...
     *   - webmail-storage — regenerated by webmail:init (WEBMAIL's component
     *     doesn't opt in)
     *
     * @return array<int, array{name: string, namespace: string, deployment: string, container: string, path: string}>
     */
    protected function backupVolumeTargets(string $kubectl): array
    {
        $targets = [
            ...
            // SeaweedFS stores volume files, not a browsable tree. Plex Commons
            // infrastructure, not a ClusterTool, so it can never be discovered
            // via ClusterTool::forDeployment() — kept as the one explicit entry.
            ['name' => 'seaweedfs', 'namespace' => 'larakube-plex', 'deployment' => 'seaweedfs', 'container' => 'seaweedfs', 'path' => '/data'],
        ];

        foreach ($this->larakubeNamespaces($kubectl) as $namespace) {
            foreach ($this->namespaceDeploymentNames($kubectl, $namespace) as $deploymentName) {
                $match = ClusterTool::forDeployment($deploymentName);

                if ($match === null || ! $match['component']->backupVolume) {
                    continue;
                }

                $key = "{$match['tool']->value}:{$match['component']->key}";

                $targets[] = [
                    'name' => self::LEGACY_VOLUME_NAMES[$key] ?? "{$match['tool']->value}-{$match['component']->key}",
                    'namespace' => $namespace,
                    'deployment' => $deploymentName,
                    'container' => $match['component']->container ?? $deploymentName,
                    'path' => (string) $match['component']->backupPath,
                ];
            }
        }
    ...
    /** Every namespace this CLI manages, the live scope backupVolumeTargets() enumerates Deployments within.
     *
     * @return list<string>
     */
    protected function larakubeNamespaces(string $kubectl): array
    {
        ...
        return array_values(array_filter(explode(' ', $out), fn (string $ns) => str_starts_with($ns, 'larakube-')));
    }
