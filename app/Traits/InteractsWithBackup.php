...
trait InteractsWithBackup
{
     ...
     *   - Bulwark's storage — regenerated by webmail:init (WEBMAIL's
     *     component doesn't opt in)
     *
     * @return array<int, array{name: string, namespace: string, deployment: string, container: string, paths: list<string>}>
     */
    protected function backupVolumeTargets(string $kubectl): array
    {
        ...
        foreach ($this->larakubeNamespaces($kubectl) as $namespace) {
            foreach ($this->namespaceDeploymentNames($kubectl, $namespace) as $deploymentName) {
                ...
                $targets[] = [
                    // The Deployment's own name — which already IS
                    // {category}-{app}-{instance} under ADR 0021, so the archive
                    // inherits the convention instead of restating it through a
                    // component key ('secrets-openbao-…', not 'secrets-app').
                    //
                    // It is also the discovery key itself, so it cannot collide:
                    // the name is a key, not a label — backup:run writes every
                    // target to "{work}/vol-{name}.tar.gz", and two targets
                    // sharing a name means the second silently overwrites the
                    // first and one instance's data never reaches the archive.
                    'name' => $deploymentName,
                    'namespace' => $namespace,
                    'deployment' => $deploymentName,
                    'container' => $match['component']->container ?? $deploymentName,
                    'paths' => $match['component']->backupPaths,
                ];
            }
        }
    ...
    protected function resolveVolumeClaim(string $kubectl, array $target): ?array
    {
        ...
        foreach ($pod['containers'] ?? [] as $container) {
            ...
            foreach ($container['volumeMounts'] ?? [] as $mount) {
                ...
                if ($claim === null || ! str_starts_with($target['paths'][0].'/', rtrim($path, '/').'/')) {
                    continue;
                }
